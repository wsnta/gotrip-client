...
function SliderDateTrend(props: Iprops) {
    ...
    useEffect(() => {
        const fetchListDate = async () => {
            try {
                setIsLoading(true)
                const res = await axios.get(`${serverHostIO}/api/list-price?startPoint=${String(StartPoint)}&endPoint=${String(EndPoint)}&DepartDate=${DepartDate}&limit=${itemsPerPage}`)
                setItems(res.data.prices);
                setMessage(res.data.message);
                settotalCount(res.data.totalCount)
                setSkip(res.data.targetSkip)
                setIsLoading(false)
                const activeItemIndex = res.data.prices.findIndex((element: any) => String(dayjs(String(element.DepartDate), 'YYYYMMDD').format('DDMMYYYY')) === String(DepartDate));
        ...
        fetchListDate()
    }, [EndPoint, StartPoint, DepartDate]);
    ...
    const handleNext = async () => {
        if (skip + itemsPerPage <= totalCount - itemsPerPage) {
            try {
                setIsLoading(true)
                const res = await axios.get(`${serverHostIO}/api/list-price?startPoint=${String(StartPoint)}&endPoint=${String(EndPoint)}&DepartDate=${DepartDate}&skipReq=${skip + itemsPerPage}&limit=${itemsPerPage}`)
                setItems(res.data.prices);
                setMessage(res.data.message);
                settotalCount(res.data.totalCount)
                setSkip(res.data.targetSkip)
            ...
            try {
                setIsLoading(true)
                const res = await axios.get(`${serverHostIO}/api/list-price?startPoint=${String(StartPoint)}&endPoint=${String(EndPoint)}&DepartDate=${DepartDate}&skipReq=${totalCount - itemsPerPage}&limit=${itemsPerPage}`)
                setItems(res.data.prices);
                setMessage(res.data.message);
                settotalCount(res.data.totalCount)
                setSkip(res.data.targetSkip)
